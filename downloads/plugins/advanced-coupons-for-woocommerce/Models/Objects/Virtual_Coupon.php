...
class Virtual_Coupon {
    ...
    public static function create_from_coupon_code( $coupon_code ) {
        $separator = ACFWP()->Plugin_Constants->get_virtual_coupon_code_separator();
        $data      = explode( $separator, $coupon_code );
    ...
    public function get_coupon() {
        if ( is_null( $this->_objects['coupon'] ) || $this->_objects['coupon']->get_id() !== $this->_data['coupon_id'] ) {
            try {
                if ( get_post_type( $this->_data['coupon_id'] ) !== 'shop_coupon' ) {
                    throw new \Exception( __( 'The provided coupon ID is invalid.', 'advanced-coupons-for-woocommerce' ) );
                }
    ...
    public function get_data_for_session() {
        return array(
            ...
            'parent_coupon_code' => $this->get_main_code(),
        );
    }
    ...
    public function get_response_for_api( $context = 'view', $date_format = '' ) {
        ...
        if ( 'edit' === $context ) {
            $date_format = \ACFWP()->Plugin_Constants->DB_DATE_FORMAT;
        }
     ...
     * @return bool True if prop was set, false otherwise.
     */
    public function set_datetime_prop( $prop, $value, $format = '' ) {
        if ( ! in_array( $prop, array( 'date_created', 'date_expire' ), true ) || ! $value ) {
            return false;
        }

        $format   = $format ? $format : \ACFWP()->Plugin_Constants->DB_DATE_FORMAT;
