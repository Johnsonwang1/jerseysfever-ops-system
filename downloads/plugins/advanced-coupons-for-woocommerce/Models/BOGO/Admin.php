...
class Admin extends Base_Model implements Model_Interface, Initiable_Interface, Activatable_Interface {
    ...
    public function display_additional_coupon_bogo_settings( $bogo_deals, $coupon ) {
        ...
        $discount_order    = $coupon->get_advanced_prop_edit( 'bogo_discount_order', 'cheapest' );
    ...
    public function ajax_save_additional_settings() {
        ...
        if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
            ...
            $discount_order    = isset( $_POST['discount_order'] ) ? sanitize_text_field( wp_unslash( $_POST['discount_order'] ) ) : 'cheapest';

            // Reset cart hash to force recalculation if bogo_discount_order is updated.
            $current_discount_order = get_post_meta( $coupon_id, $this->_constants->META_PREFIX . 'bogo_discount_order', true );
            if ( $current_discount_order !== $discount_order ) {
                $random_string = wp_generate_password( 12, false );
                \WC()->session->set( 'acfw_bogo_cart_hash', $random_string );
            }

            update_post_meta( $coupon_id, $this->_constants->META_PREFIX . 'bogo_auto_add_products', (int) $auto_add_products );
            update_post_meta( $coupon_id, $this->_constants->META_PREFIX . 'bogo_discount_order', $discount_order );
